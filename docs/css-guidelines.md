...
`willa` 是聚合包，只通过 `auklet.config.mjs` 组合 content 和 widgets 的 CSS，不维护自己的主题变量。
...
跨包 CSS 依赖通过 `auklet.config.mjs` 表达。
...
不需要。通过 `auklet.config.mjs` 的 `styles.dependencies` 声明依赖，让构建产物组合 content CSS。
