...
基础组件后续仍需要补齐表单、表格和评论讨论能力。这些组件不是 AI 专属，
应归 `@willa-ui/content`，由 AI 包、文档站、博客、内容平台和产品后台按场景组合使用。
...
| 优先级 | 能力       | 当前状态                               | 待补组件                                                          |
| ------ | ---------- | -------------------------------------- | ----------------------------------------------------------------- |
| P0     | 表单       | 已有 `Input`、`TextArea`、`RangeInput` | `FormField`、`Checkbox`、`Radio`、`Switch`、`Select`、`FormGroup` |
| P0     | 表格       | 暂无基础表格组件                       | `Table`                                                           |
...
`TextArea` 和 `RangeInput`，但还缺布尔选择、单选、多选、下拉选择和字段布局。
优先补轻量 UI 底座，不内置完整表单状态管理和校验框架。
...
- `Table`：基础表格结构，支持表头、表体、空态、加载态、紧凑密度和横向滚动。
- `DataTable`：基于 columns 和 rows 的数据表格，后续再评估排序、选择、
  操作列、分页和列宽控制。
