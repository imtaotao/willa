...
- `PromptInput`：AI 输入入口。基础版支持多行输入、快捷提交、提交状态、
  ...
  `actions` 组合扩展。
- `ChatMessage`：单条消息。已支持 user、assistant、system、tool 角色、
  默认对齐、头像、状态、操作区和 footer；Markdown、代码块、引用、复制、
  重新生成和反馈可以通过 `children` 与 `actions` 组合扩展。
...
- `MessageActions`：消息操作区。支持复制、重新生成、点赞、点踩、编辑并重试等操作，
  通过 `ChatMessage.actions` 组合使用。
- `ThinkingIndicator`：生成和任务状态提示。支持 thinking、searching、reading、generating 等状态文案。
- `SuggestionChips`：快捷提示词。用于推荐问题、任务入口和轻量 prompt 建议。

### 结果与过程

- `SourceCard`：来源卡片。用于 AI 搜索、RAG、文档问答，展示标题、URL、片段、
  来源类型和跳转入口。
- `Citation`：行内引用。用于 AI 回复中的脚注、证据编号和来源悬浮说明。
- `ToolCallCard`：工具调用展示。支持 pending、success、error 状态，
  用于搜索、读取文件、执行命令和查询数据库等过程。
- `AttachmentList`：上下文附件。展示文件、图片、网页链接、表格和代码片段，
  可复用 `FileCard`、`Image`、`UrlLink` 等基础组件。
...
- `FileCard`、`FileTree`、`Download`、`Image`、`ImageGallery` 可以支撑文件上下文、附件下载、附件预览、目录选择和多媒体回答。
...
1. P0：`ThinkingIndicator`、`MessageActions`、`ToolCallCard`、`SourceCard`、
   `AttachmentList`。
