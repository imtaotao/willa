...
const PromptInputMentionPreview = () => {
  const [value, setValue] = useState("");
  const [lastPrompt, setLastPrompt] = useState("暂无提及内容");

  return (
    <div style={stackStyle}>
      <PromptInput
        value={value}
        placeholder="输入问题，输入 @、#、$ 提及上下文"
        users={mentionUsers}
...
export default defineDoc({
  ...
  name: "PromptInput",
  category: "ai",
  packageName: "willa/PromptInput",
  description: "用于 AI 对话、智能搜索和生成任务的提示词输入区域。",
  imports: [{ name: "PromptInput", from: "willa/PromptInput" }],
  css: "willa/PromptInput.css",
  demo: {
    name: "PromptInputPreview",
  ...
  sections: [
    ...
    },
    {
      ...
      `,
      content: <PromptInputMentionPreview />,
    },
  ],
  propGroups: [
    {
      title: "基础能力",
      description: "PromptInput 的基础输入与提交能力。",
    },
    {
      title: "提及能力（透传至 MentionInput）",
      description: "以下属性用于 @/#/$ 提及入口和候选列表，来自 MentionInput。",
    },
  ],
  props: [
    {
      name: "value",
      type: "string",
      description: "受控输入值，保持原生 textarea 语义。",
    },
    {
      name: "defaultValue",
      type: "string",
    ...
    },
    {
      ...
      group: "提及能力（透传至 MentionInput）",
      description: "覆盖单条提及项渲染。",
    },
    {
      name: "onMentionClick",
      type: "() => void",
      group: "提及能力（透传至 MentionInput）",
      description: "点击提及入口按钮时触发。",
    },
  ],
});
