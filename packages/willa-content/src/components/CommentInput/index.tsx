...
export type CommentInputMentionListProps = Omit<
  ListProps,
  "items" | "children"
>;
...
const getMentionItemMeta = (option: CommentInputMentionItem): string | null => {
...
const COMMENT_INPUT_MENTION_VIRTUAL_SCROLL_ITEM_HEIGHT = 54;
...
export type CommentInputProps = {
  ...
  mentionListProps?: CommentInputMentionListProps;
...
export function CommentInput(props: CommentInputProps) {
  ...
  const updateMentionPanelPosition = useCallback(() => {
    ...
    const targetWidth = getCommentInputMentionPanelWidth({
    ...
    const leftLimitMin = textareaRect ? Math.max(8, textareaRect.left + 6) : 8;
  ...
  const filteredMentions = useMemo(() => {
    ...
    const uniqueOptions = new Map<string, CommentInputMentionItem>();
  ...
  const openMentionPanel = () => {
    if (!mentionOptions.length || !textareaRef.current || disabled || loading) {
      return;
    }

    const cursor = textareaRef.current.selectionStart ?? currentValue.length;
    const trigger = mentionTriggers[0];
    if (!trigger) return;
    ...
    const panelStyle = {
      ...
      width:
        mentionPanelPosition?.width !== undefined
          ...
          : `${getCommentInputMentionPanelWidth({
      ...
      "--willa-comment-input-mention-item-bg-hover":
        textareaStyle
          ...
          ?.trim() ||
        textareaStyle?.getPropertyValue("--willa-list-item-bg-hover")?.trim() ||
        "rgba(255, 255, 255, 0.08)",
      "--willa-comment-input-mention-item-border-hover":
        textareaStyle
          ?.getPropertyValue("--willa-comment-input-mention-item-border-hover")
          ?.trim() ||
          ...
          <div className={mentionPanelClassName}>
            {filteredMentions.map((option) => (
              <div key={option.id ?? option.value}>
                {renderMentionItem(activeMention, option, handleMentionSelect)}
              </div>
            ))}
          </div>
        </div>,
        ...
        <div className={classNames(mentionPanelClassName, externalClassName)}>
          <List
            ...
            virtualScrollItemHeight={
              COMMENT_INPUT_MENTION_VIRTUAL_SCROLL_ITEM_HEIGHT
            }
  ...
  const showTools = Boolean(mentionOptions.length || onMentionClick || actions);
...
const getMentionOptionDedupKey = (option: CommentInputMentionItem) => {
...
const getCommentInputMentionPanelWidth = (options: {
  isMobile: boolean;
