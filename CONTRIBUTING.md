...
- `packages/willa/auklet.config.mjs` 如组件扫描配置发生变化
...
- widgets 依赖 content 的 CSS 和主题时，通过 `auklet.config.mjs` 的 `styles.dependencies` 表达，不复制 content 变量。
